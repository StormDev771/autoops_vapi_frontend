...
export default function BuildAgent() {
  ...
  // Persist agentId in localStorage
  ...
  // Keep agentId in sync with localStorage
  useEffect(() => {
    if (agentId) {
  ...
  const handleGenerateAgent = async () => {
    ...
    try {
      const res = await api.post("/vapi/agent/create");
      if (res.status === 200) {
        setAgentId(res.data.agent.id);
        toast.success(res.data.message);
      } else {
        toast.error("Failed to generate voice agent");
      }
    } catch (error) {
      console.error("Error generating agent:", error);
      toast.error("An error occurred while generating the agent.");
    } finally {
      setIsGenerating(false);
    }
  };

  const handleAssignPhoneNumber = async (agentIdParam: string | null) => {
    setIsAssigning(true);
    if (!agentIdParam) {
      toast.error("Please generate an agent first.");
      setIsAssigning(false);
      return;
    }
    try {
      const res = await api.post("/vapi/agent/assignTn", {
        agentId: agentIdParam,
      });
      if (res.status === 200) {
        toast.success(
          `${res.data.message} The phone number is ${res.data.phone.number}.`
        );
        localStorage.removeItem("agentId");
        setAgentId(null);
      } else {
      ...
      toast.error("An error occurred while assigning the phone number.");
    } finally {
      setIsAssigning(false);
    }
  };
  ...
  return (
    <AgentIdContext.Provider value={{ agentId, setAgentId }}>
                  ...
                  <Card className="glass-morphism hover:scale-105 transition-transform duration-300">
                    ...
                    <CardContent>
                      <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <Button
                          variant="secondary"
                          className="flex-1 flex items-center justify-center gap-2 transition-colors duration-200 hover:bg-purple-100 hover:text-purple-700"
                          onClick={() => handleAssignPhoneNumber(agentId)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
                  ...
                  <Card className="glass-morphism text-center">
                    <CardContent className="p-6">
                      ...
                      <p className="text-muted-foreground text-sm">
                        Seamless integration across all major e-commerce
                        platforms
                      </p>
                    </CardContent>
                  </Card>
